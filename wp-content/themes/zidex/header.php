...
<head>
    ...
    <?php if ( ! function_exists( 'has_site_icon' ) || ! has_site_icon() ) { ?>
        <link rel="shortcut icon" href="<?php echo esc_url(zidex_redux('mt_favicon', 'url')); ?>">
    <?php } ?>
...
<body <?php body_class(); ?>>
    ...
    $header_layout = zidex_redux('mt_header_layout');
            ...
            <div class="fixed-search-inside">
                <div class="modeltheme-search">
                    <form method="GET" action="<?php echo esc_url(home_url('/')); ?>">
                        <input class="search-input" placeholder="<?php echo esc_attr__('Enter search term...', 'zidex'); ?>" type="search" value="" name="s" id="search" />
                ...
                <div class="header7 sidebar-content">
                    ...
                    <div class="left-side">
                        <?php if ( class_exists( 'ReduxFrameworkPlugin' ) ) { ?>
                            ...
                            <?php if(zidex_redux('mt_header_fixed_sidebar_menu_site_title_status') == 'site_logo'){ ?>
                                <?php if(zidex_redux('mt_logo','url')){ ?>
                                    <h1 class="logo">
                                        <a href="<?php echo esc_url(get_site_url()); ?>">
                                            <img width="200" src="<?php echo esc_url(zidex_redux('mt_logo','url')); ?>" alt="<?php echo esc_attr(get_bloginfo()); ?>" />
                                        </a>
                                    </h1>
                                <?php } ?>
                            <?php }else{ ?>
                        ...
                        <?php if (is_active_sidebar( zidex_redux('mt_header_fixed_sidebar') )) {
                            dynamic_sidebar( zidex_redux('mt_header_fixed_sidebar') ); 
                        } ?>
                    </div>
                </div>
            </div>
    ...
    <div id="page" class="hfeed site">
        <?php
            ...
            if (in_array($header_layout, $below_slider_headers)){
                ...
                echo wp_kses_post(zidex_current_header_template());
